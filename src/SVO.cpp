...
void SVO::Build(const std::vector<float>& heightMap)
{
	...
	Logger::Get()->Log(Logger::INFO, "\tTexture 3D would take:   %d MB", _Size * _Size * _Size / 1024 / 1024);
...
int SVO::Build_Internal(const std::vector<float>& heightMap, uint16_t depth, uint16_t rootX, uint16_t rootY, uint16_t rootZ)
{
	...
	// float scaleFactor = _Size / 2;
    // float shiftFactor = _Size / 2;
	// int FirstElement = heightMap[rootX + rootZ * MySize] * scaleFactor + shiftFactor >= rootY ? 1 : 0;
	// for (uint16_t x = rootX; x < rootX + MySize; x++)
	// {
	// 	for (uint16_t z = rootZ; z < rootZ + MySize; z++)
	// 	{
	// 		for (uint16_t y = rootY; y < rootY + MySize; y++)
	// 		{
	// 			int ActualElement = heightMap[x + z * MySize] * scaleFactor + shiftFactor >= y ? 1 : 0;
	// 			if (ActualElement != FirstElement)
	// 			{
	// 				IsSame = false;
	// 				break;
	...
	int FirstElement = (rootX % 2 == 0 && rootY % 2 == 0 && rootZ % 2 == 0) ? 1 : 0;
	IsSame = MySize == 1 ? true : false;
